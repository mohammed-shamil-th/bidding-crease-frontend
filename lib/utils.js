...
// Get category icon from tournament categories
export const getCategoryIcon = (player, tournament) => {
  if (!player || !player.category || !tournament || !Array.isArray(tournament.categories)) {
    return null;
  }
  
  const category = tournament.categories.find(
    (c) => c && c.name && c.name === player.category
  );
